...
$Schema = Get-jsonSchema ('Set-CallQueueAgentAlertTime')
...
Else {
    ...
    If (-Not $Result){
        ...
        $CallQueueAgentAlertTime = $Request.Body.AgentAlertTime        
    }    
}
...
If ($StatusCode -eq [HttpStatusCode]::OK) {
    Try {
            $Resp = Set-CsCallQueue -Identity $CallQueueId.Identity -AgentAlertTime $CallQueueAgentAlertTime -ErrorAction Stop
        }
    Catch {
        $Resp = @{ "Error" = $_.Exception.Message }
        $StatusCode =  [HttpStatusCode]::BadGateway
