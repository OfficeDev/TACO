...
Else {
    ...
    If (-Not $Result){
        ...
        $CallQueueGreeting= $Request.Body.Greeting
        $CallQueueGreetingType= $Request.Body.GreetingType
...
If ($StatusCode -eq [HttpStatusCode]::OK) {
    Try {
        If ($CallQueueGreeting -ne $null -and $CallQueueGreetingType -eq "Custom")
        {
            $authHeader = Get-AuthenticationToken
            $audioPrompt = Upload-AudioPrompt -Token $authHeader -Site $Site -Path $CallQueueName -FileName $CallQueueGreeting -Type HuntGroup
            $Resp = Set-CsCallQueue -Identity $CallQueueId.Identity -WelcomeMusicAudioFileId $audioPrompt.id  -ErrorAction Stop
        }
        ElseIf ($CallQueueGreeting -eq $null -and $CallQueueGreetingType -eq "Default")
        {
            $Resp = Set-CsCallQueue -Identity $CallQueueId.Identity -WelcomeMusicAudioFileId $null  -ErrorAction Stop
        }                                                        
