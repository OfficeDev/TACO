...
$Schema = Get-jsonSchema ('Set-CallQueueOverflowThreshold')
...
Write-Output $CallQueueId.Identity

# Updating call overflow action
If ($StatusCode -eq [HttpStatusCode]::OK) {
    Try {
        $Resp = Set-CsCallQueue -Identity $CallQueueId.Identity -TimeoutThreshold $TimeoutThreshold -ErrorAction Stop
    }
    Catch {
        $Resp = @{ "Error" = $_.Exception.Message }
        $StatusCode =  [HttpStatusCode]::BadGateway
